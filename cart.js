...
const addItemToCart = (name, price, imageSrc) => {
  ...
  if (existingItem) {
    ...
    let oldQty = parseInt(existingItem.dataset.quantity);
    ...
    // Update the display
    const priceElement = existingItem.querySelector(".total-price");
    priceElement.textContent = `$${totalPrice.toFixed(2)}`;

    const itemDetails = existingItem.querySelector(".item-details > div:last-child");
  itemDetails.innerHTML = `
    <p>$${price.toFixed(2)} x ${newQty}</p>
  ...
  `;
    ...
    cartItem.classList.add("cart-item");
    cartItem.dataset.quantity = count;
    cartItem.dataset.name = name; // Used to find existing item
    cartItem.innerHTML = `
      <img src="${imageSrc}" alt="${name}" />
      <div class="item-details" style="margin-bottom: none; padding-bottom:none;">
        <div>${name}</div>
        <div>
